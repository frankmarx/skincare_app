...
Respond ONLY in valid JSON, no markdown:
{{"skinProfile":{{"type":"...","concerns":["..."],"summary":"..."}},"amRoutine":[{{"step":1,"productName":"...","type":"...","tip":"...","frequency":"..."}}],"pmRoutine":[{{"step":1,"productName":"...","type":"...","frequency":"..."}}],"warnings":["..."],"weeklyFrequency":[{{"productName":"...","frequency":"...","note":"..."}}]}}'''
            }]
        )
        import json
        return json.loads(response.content[0].text.replace('```json', '').replace('```', '').strip())
